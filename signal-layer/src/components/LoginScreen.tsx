  ...
  return (
    <div className="h-full w-full flex items-center justify-center bg-[var(--bg-primary)] relative overflow-hidden">
      ...
      <form onSubmit={submit} className="relative w-[420px] rounded-2xl border border-[var(--border-accent)] bg-[rgba(18,21,28,.96)] p-9 shadow-[0_28px_80px_rgba(0,0,0,.48)]">
        ...
        <p className="mt-2 text-[12px] text-[var(--text-muted)]">{mode === 'login' ? '登录后继续管理指标、策略与选股任务' : '注册后将自动获得普通用户角色'}</p>
        ...
        <div className="mt-6 pt-5 border-t border-[var(--border-primary)] text-center"><button type="button" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError('') }} className="text-[12px] text-[var(--text-muted)] hover:text-[var(--text-primary)]">{mode === 'login' ? '还没有账户？创建账户' : '已有账户？返回登录'}</button></div>
      </form>
    </div>
  )
}
