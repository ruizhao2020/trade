...
export function SymbolSelector({ market, symbol, symbolName, onMarketChange, onSymbolChange }: Props) {
  ...
  // 市场目录由后端给出（已按 enabled 过滤并按 sort_order 排序），前端不再维护硬编码兜底列表
  ...
  useEffect(() => {
    fetchMarkets()
      ...
      .catch((error: unknown) => {
        setMarkets([])
        setMarketsError(error instanceof Error ? error.message : '市场列表加载失败')
      })
  }, [])

  const activeMarket = markets.find((item) => item.id === market)
  const defaultSymbol = activeMarket?.default_symbol ?? ''
  const defaultHint = defaultSymbol ? `${activeMarket?.default_symbol_name || ''} ${defaultSymbol}`.trim() : ''
  // 默认标的已在候选列表里时不再重复展示入口
  const defaultChip = Boolean(defaultSymbol) && defaultSymbol !== symbol && !options.some((item) => item.symbol === defaultSymbol)
  ...
  const loadSymbols = useCallback(async (kw?: string) => {
    ...
    try {
      ...
      // 请求失败与“确实没有匹配结果”必须区分，否则用户会以为标的池是空的
      setOptions([])
      setOptionsError(error instanceof Error ? error.message : '标的列表加载失败')
    } finally {
      setLoading(false)
    }
  }, [market])
  ...
  /** 选中后端为该市场指定的默认标的（仍需用户点击，不自动加载行情） */
  const handleSelectDefault = () => {
    if (!activeMarket?.default_symbol) return
    onSymbolChange(activeMarket.default_symbol, activeMarket.default_symbol_name || '')
    setOpen(false)
    setKeyword('')
  }
  ...
  return (
    <div className="flex items-center gap-2.5 relative shrink-0" ref={selectorRef}>
      ...
      {marketsError && (
        <span title={marketsError} className="text-[11px] text-[var(--accent-red)] shrink-0">市场列表加载失败</span>
      )}
      ...
      <div className="relative shrink-0">
        <input
          ...
          disabled={!market}
          placeholder={!market ? '请先选择市场' : symbolName ? `${symbolName}  ${symbol}` : defaultHint ? `搜索标的（默认 ${defaultHint}）` : '请选择标的'}
          aria-label="搜索并选择标的"
          ...
          >
            {loading ? (
              ...
              <div className="px-4 py-5 text-center">
                <div className="text-[12px] text-[var(--accent-red)]">标的列表加载失败</div>
                <div className="mt-1 text-[11px] text-[var(--text-muted)]">{optionsError}</div>
                <button type="button" onClick={() => void loadSymbols(keyword)} className="mt-2.5 text-[11px] text-[var(--accent)] hover:underline">重新加载</button>
              </div>
            ) : options.length === 0 && !defaultChip ? (
              <div className="px-4 py-5 text-[12px] text-[var(--text-muted)] text-center">{keyword ? '无匹配结果' : '该市场暂无可选标的'}</div>
            ) : (
              <>
                {options.map(item => (
                  <button
                  ...
                  >
                    ...
                    <span className="flex-1 min-w-0">
                      ...
                      <span className="block text-[11px] text-[var(--text-muted)] mt-0.5 truncate">{item.industry || item.exchange || item.market}</span>
                    </span>
                ...
                {defaultChip && (
                  <button
                    ...
                    className="w-full min-h-11 text-left px-4 py-2.5 hover:bg-[var(--bg-tertiary)] transition-colors flex items-center gap-3"
                  >
                    <span className="w-2 h-2 rounded-full shrink-0 border border-[var(--accent)]" />
                    <span className="flex-1 min-w-0">
                      <span className="block text-[12px] font-medium text-[var(--text-primary)] truncate">{activeMarket?.default_symbol_name || activeMarket?.default_symbol}</span>
                      <span className="block text-[11px] text-[var(--text-muted)] mt-0.5">{activeMarket?.name}默认标的</span>
                    </span>
                    <span className="text-[var(--text-secondary)] font-mono text-[11px] shrink-0">{activeMarket?.default_symbol}</span>
                  </button>
                )}
              </>
            )}
