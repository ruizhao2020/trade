...
function NotificationAdminPanel({
  ...
  async function addChannel() {
    if (!channelName.trim()) { setActionTone('error'); setActionMessage('请填写渠道名称'); return }
    if (channelType !== 'browser' && !webhook.trim()) { setActionTone('error'); setActionMessage('企业微信和飞书需要填写 Webhook 地址'); return }
    setSaving(true)
    setActionMessage('')
    try {
      ...
      onChannelsChange([...channels, item]); setWebhook(''); setActionTone('ok'); setActionMessage('渠道已发布')
    } catch (error) {
      setActionTone('error')
  ...
  return <div className="max-w-6xl mx-auto space-y-4">
    <section className="p-5 rounded-xl border border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[0_8px_24px_rgba(0,0,0,.08)]">
      ...
      {actionMessage && <div role="status" className={`mt-2 text-[11px] ${actionTone === 'ok' ? 'text-[var(--accent-green)]' : 'text-[var(--accent-red)]'}`}>{actionMessage}</div>}
      <div className="mt-4 divide-y divide-[var(--border-primary)]">{channels.map((channel) => <div key={channel.id} className="py-3 flex items-center gap-3"><span className={`w-2 h-2 rounded-full ${channel.enabled ? 'bg-[var(--accent-green)]' : 'bg-[var(--text-muted)]'}`} /><div className="flex-1"><div className="text-[12px]">{channel.name}</div><div className="text-[11px] text-[var(--text-muted)] mt-1">{channel.channel_type === 'wecom' ? '企业微信' : channel.channel_type === 'feishu' ? '飞书' : '浏览器'} · {channel.webhook_url_masked || '无 Webhook'}</div></div><Toggle value={channel.enabled} onChange={(enabled) => void updateNotificationChannelForAdmin(channel.id, { enabled }).then((next) => onChannelsChange(channels.map((item) => item.id === next.id ? next : item))).catch((error: unknown) => { setActionTone('error'); setActionMessage(error instanceof Error ? error.message : '渠道状态更新失败') })} /><button type="button" onClick={() => { if (!window.confirm(`删除渠道“${channel.name}”？已引用该渠道的推送任务将不再送达。`)) return; void deleteNotificationChannelForAdmin(channel.id).then(() => onChannelsChange(channels.filter((item) => item.id !== channel.id))).catch((error: unknown) => { setActionTone('error'); setActionMessage(error instanceof Error ? error.message : '渠道删除失败') }) }} className="text-[11px] text-[var(--text-muted)] hover:text-[var(--accent-red)]">删除</button></div>)}</div>
    </section>
...
function TemplateEditor({ template, onChange, onSaved }: { template: AdminNotificationTemplate; onChange: (template: AdminNotificationTemplate) => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-primary)]"><div className="flex items-center gap-3"><input value={name} onChange={(event) => setName(event.target.value)} className="flex-1 h-8 px-2 rounded-md bg-[var(--bg-secondary)] border border-[var(--border-primary)] text-[11px]" /><span className="text-[11px] text-[var(--text-muted)]">{NOTIFICATION_EVENT_LABEL[template.event_type] || template.event_type}</span><Toggle value={template.enabled} onChange={(enabled) => void updateNotificationTemplateForAdmin(template.id, { enabled }).then((next) => { onChange(next); return onSaved() }).catch((error) => setMessage(error instanceof Error ? error.message.replace(/^Error:\s*/, '') : '更新失败'))} /></div><textarea value={content} onChange={(event) => setContent(event.target.value)} className="mt-2 w-full min-h-24 p-2 rounded-md bg-[var(--bg-secondary)] border border-[var(--border-primary)] text-[11px] leading-5 outline-none focus:border-[var(--accent)]" /><div className="mt-2 flex items-center justify-end gap-3">{message && <span role="status" className={`text-[11px] ${message === '已保存' ? 'text-[var(--accent-green)]' : 'text-[var(--accent-red)]'}`}>{message}</span>}<button type="button" disabled={saving} onClick={() => void save()} className="h-7 px-3 rounded-md bg-[var(--accent)] text-white text-[11px] disabled:opacity-50">{saving ? '保存中…' : '保存模板'}</button></div></div>
}
          ...
          return <div key={item.id}>
            <div className="min-h-14 px-5 py-3 flex items-center gap-5">
              <button type="button" onClick={() => toggleExpanded(item.indicator_type)} className="min-w-0 flex-1 flex items-center gap-2 text-left">
                ...
                <code className="text-[11px] text-[var(--text-muted)]">{item.indicator_type}</code>
                {childFeatures.length > 0 && <span className="text-[11px] text-[var(--text-muted)]">{childFeatures.filter((feature) => feature.public_visible).length}/{childFeatures.length} 子项</span>}
              </button>
            ...
            {expanded && <div className="px-5 pb-4 ml-5">
              <div className="flex items-center gap-3 mb-3"><span className="text-[11px] text-[var(--text-muted)]">公开名称</span><input defaultValue={item.display_name} onBlur={(event) => { if (event.target.value !== item.display_name) void updateIndicator(item, { display_name: event.target.value }) }} className="h-8 w-56 px-2 rounded-md border border-[var(--border-primary)] bg-[var(--bg-tertiary)] text-[11px]" /><button type="button" onClick={() => void addFeature(item.indicator_type)} className="ml-auto h-7 px-3 rounded-md border border-[var(--border-primary)] text-[11px] text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]">添加子指标</button></div>
              <div className="rounded-lg border border-[var(--border-primary)] bg-[var(--bg-primary)] divide-y divide-[var(--border-primary)]">
                {childFeatures.length === 0 && <div className="px-4 py-3 text-[11px] text-[var(--text-muted)]">该指标暂无可单独配置的子指标</div>}
                {childFeatures.map((feature) => <div key={feature.id} className="min-h-12 px-4 py-2 flex items-center gap-5">
                  <div className="min-w-0 flex-1"><input defaultValue={feature.display_name} onBlur={(event) => { if (event.target.value !== feature.display_name) void updateFeature(feature, { display_name: event.target.value }) }} className="h-8 w-56 max-w-full px-2 rounded-md border border-[var(--border-primary)] bg-[var(--bg-tertiary)] text-[11px]" /><code className="ml-2 text-[11px] text-[var(--text-muted)]">{feature.feature_code}</code></div>
                  <div className="flex items-center gap-8 shrink-0">
            ...
            {modules.map((module) => (
              <section key={module.id} className="p-5 rounded-xl border border-[var(--border-primary)] bg-[var(--bg-secondary)] shadow-[0_8px_24px_rgba(0,0,0,.08)]">
                <div className="grid grid-cols-[1fr_160px_auto_auto_auto] gap-5 items-center">
                  <div><div className="text-[13px] font-semibold">{module.name}</div><code className="mt-1 block text-[11px] text-[var(--text-muted)]">{module.code}</code></div>
                  <label className="text-[11px] text-[var(--text-muted)]">排序<input type="number" defaultValue={module.sort_order} onBlur={(event) => { if (Number(event.target.value) !== module.sort_order) void updateModule(module.id, { sort_order: Number(event.target.value) }).then(() => { void load(); onModulesChanged() }) }} className="ml-2 w-16 h-9 px-2 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-primary)] text-[12px]" /></label>
