...
function ChartStage({ loading, error, klines, analysis, options, indicators, onCursor, detailFeatures }: { loading: boolean; error: string | null; klines: RawKline[]; analysis: ChanAnalysis | null; options: ChanRenderOptions; indicators: IndicatorResult[]; onCursor: (time: number | null) => void; detailFeatures: string[] }) {
  return <div className="flex-1 relative min-w-0 min-h-0 bg-[var(--chart-background)]">{loading ? <div className="absolute inset-0 grid place-items-center text-[12px] text-[var(--text-muted)]">加载中…</div> : error ? <div className="absolute inset-0 grid place-items-center text-[12px] text-[var(--accent-red)]">{error}</div> : klines.length ? <Chart klineData={klines} chanAnalysis={analysis ?? undefined} chanOptions={options} indicatorResults={indicators} onCursorTimeChange={onCursor} chanDetailFeatures={detailFeatures} /> : <div className="absolute inset-0 grid place-items-center text-[12px] text-[var(--text-muted)]">无数据</div>}</div>
}
...
export function PublicApp() {
  ...
  const handleMarket = useCallback((next: string) => { setMarket(next); setCursorTime(null) }, [])
  ...
  return <div className="flex h-full w-full overflow-hidden bg-[var(--bg-primary)]"><PublicRail /><main className="flex-1 min-w-0 flex flex-col overflow-hidden"><WorkbenchHeader title="指标分析" {...headerProps} trailing={dataStatusBadge} /><IndicatorWorkbenchToolbar selectedIndicators={selected} onIndicatorChange={setSelected} chanOptions={chanOptions} onChanChange={setChanOptions} analysis={analysis ?? undefined} visibleChanFeatures={visibleChanFeatures} chanFeatureLabels={chanFeatureLabels} /><div className="flex flex-1 min-h-0 overflow-hidden"><ChartStage loading={loading} error={error} klines={klines} analysis={analysis} options={chanOptions} indicators={results} onCursor={setCursorTime} detailFeatures={detailChanFeatures} /><IndicatorInfoPanel collapsed={collapsed} onToggle={() => setCollapsed((value) => !value)} klineData={klines} analysis={analysis ?? undefined} chanOptions={chanOptions} indicators={selected} results={results} cursorTime={cursorTime} visibleChanFeatures={visibleChanFeatures} visibleIndicatorDetails={visibleIndicatorDetails} /></div></main></div>
}
