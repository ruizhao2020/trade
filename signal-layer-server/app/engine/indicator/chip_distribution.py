...
def _profile_metrics(
    ...
    peak_price = prices[max(range(len(prices)), key=lambda index: weights[index])]
...
class ChipDistributionCalculator(IndicatorCalculator):
    ...
    @staticmethod
    def _params(params: dict[str, Any]) -> dict[str, int]:
        normalized = {
    ...
    def _calculate_daily(self, klines: list[dict], params: dict[str, int]) -> IndicatorResult:
        ...
        for bar in bars:
            ...
            metrics = _profile_metrics(
                ...
                processed_valid / max(1, len(snapshots) + 1) * 100, processed_valid, 0,
            )
            snapshots.append(self._snapshot(bar, profile, metrics))
        return self._result(params, prices, snapshots)

    def _calculate_intraday(
        self, intraday_klines: list[dict], daily_klines: list[dict], params: dict[str, int],
    ) -> IndicatorResult:
    ...
    def _result(
        self, params: dict[str, int], prices: list[float], snapshots: list[ProfileSnapshot],
    ) -> IndicatorResult:
        latest = snapshots[-1]
        values = [{
            "time": float(latest.time), "price": round(price, 8), "weight": weight,
            "is_profit": 1.0 if price <= latest.metrics["current_price"] else 0.0,
            **latest.metrics,
        } for price, weight in zip(prices, latest.weights)]
