#!/usr/bin/env bash
# SignalLayer 私有入口一键启动脚本（前端 + 后端）
#
# 用法: ./start-private.sh [start|stop|restart|status]
#       ./start-private.sh backend  start|stop
#       ./start-private.sh frontend start|stop
#
# 前端 npm run dev:private (127.0.0.1:4175)；后端 uvicorn app.main:app (0.0.0.0:8000)。
...
FRONTEND_PID_FILE=/tmp/signal-layer-frontend-private.pid
BACKEND_LOG=/tmp/signal-layer-server.log
FRONTEND_LOG=/tmp/signal-layer-frontend-private.log
...
start_frontend() {
  ...
  log "启动前端: npm run dev:private -- --host $FRONTEND_HOST --port $FRONTEND_PORT"
  log "日志: $FRONTEND_LOG"
  (
    cd "$FRONTEND_DIR"
    nohup npm run dev:private -- --host "$FRONTEND_HOST" --port "$FRONTEND_PORT" \
      >>"$FRONTEND_LOG" 2>&1 &
    echo $! >"$FRONTEND_PID_FILE"
  )

  if wait_http "http://$FRONTEND_HOST:$FRONTEND_PORT/" 30 1; then
    log "前端就绪: http://$FRONTEND_HOST:$FRONTEND_PORT/ (private)"
  else
...
status() {
  ...
  if port_in_use "$FRONTEND_PORT"; then
    echo "前端:  运行中 (PID $(pid_of_port "$FRONTEND_PORT"))  http://$FRONTEND_HOST:$FRONTEND_PORT  [private]"
  else
    echo "前端:  未运行"
  fi
}
